...
search/build functionality for the LLM gods.
"""
...
def search_schematics(query: str) -> str:
    ...
    if not results:
        return (f"No schematics found matching '{query}'. "
                f"Try broader or different search terms.")
    ...
    ids = [bp['id'] for _, _, bp in top]
    lines.append(f"\nValid blueprint IDs: {', '.join(ids)}")
    lines.append("Pick the best match and use build_schematic with one of these IDs. Do NOT search again.")
    return "\n".join(lines)
